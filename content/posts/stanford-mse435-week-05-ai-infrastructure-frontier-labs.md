...
同步训练是一个典型的电网问题：数十万 GPU 必须同时工作，任何一个节点的故障或功率波动都会影响整个训练作业。这一负载形态与部分可分散调度的推理请求不同，但不能据此认为所有推理负载都具备相同弹性。

他还区分了三种量级的算力扩张状态：30 GW 是讲者描述的愿景目标，约 100 GW 是他对美国 hyperscaler 规划的即席估算，7 TW 是“每人一块 GPU 等效智能”的思想实验。三者面对的工程和政策约束完全不同。
...
异构计算由此成为一种设计方向——不同计算图节点有不同的计算特征（prefill 是计算密集、decode 是内存带宽密集、工具调用可能是 I/O 密集），用单一硬件服务所有节点不是最优。
