...
- 历史文章保留未标记，不根据文章类型、发布时间、已生成笔记或 PDF 推断是否学过。`done` 记录完成过一轮学习，不表示完全掌握。
- 单独修改学习状态保留 `date` 和 `lastmod`；修订笔记或 PDF 不重置已有状态。

侧栏的“学习清单”进入 `/learning/`，并显示待学习数量。清单默认展开待学习，已学习和未标记折叠；首页及普通文章列表保持原有阅读样式。从清单打开文章时，链接带 `?learning=1`，正文底部显示学习状态、“在 GitHub 编辑”和“返回学习清单”。这个参数只控制学习记录的显示，不提供隐私或访问控制。

可在本地修改 Markdown，也可通过文章底部的 GitHub 编辑入口修改同一文件。提交并推送后，等待 GitHub Pages 部署成功，各设备访问网页即可看到更新；浏览器不保存学习状态，站点无需数据库或后端。仓库和学习记录均公开，编辑权限由 GitHub 管理。
