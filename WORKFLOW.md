...
| **学习记录** | 文章 frontmatter + 独立学习清单 | `learning_status` 保存个人状态，提交后随 Pages 部署跨设备同步 |
...
新建文章时设置 `date` 和 `lastmod`，修订已有文章保留首次发布的 `date`。纯栏目、标签、学习状态或格式整理不修改 `lastmod`；只有标题、摘要、description 或正文发生变化时才更新内容时间。`视频笔记` 不再写入 tags，历史标签入口转向同名栏目，原分页与 RSS 继续可用。

首页按时间倒序展示文章，每页 10 篇；新读者引导链接到阅读专题，主题入口链接到主题浏览页的对应分组，栏目链接进入对应 taxonomy 页面继续分页浏览。首页、栏目、标签详情共用文章条目和分页模板，并为每个分页生成指向自身的 canonical。文章条目突出标题与摘要，类型、日期、阅读时长和至多两个主题标签放在次要位置。左侧提供最新文章、主题浏览、阅读专题、学习清单、归档和搜索入口；文章页可切换专注阅读。

## 个人学习记录

学习状态保存在 `content/posts/<slug>.md` 的 frontmatter，由 GitHub 仓库作为唯一记录。字段可选，填写时只允许以下两个字符串：

```yaml
learning_status: pending  # 待学习；完成一轮阅读或学习后改为 done
```

- `pending` 表示待学习，`done` 表示已学习；缺少字段表示未标记。不要写空值或 `unmarked`。
- 新增视频笔记默认写入 `pending`；确认用户已学习后可以写 `done`。普通文章只在用户确认已读后写 `done`，未确认时省略字段；也可按用户要求加入待学习。
- 未经用户确认，历史文章保留未标记，不根据文章类型、发布时间、已生成笔记或 PDF 推断是否学过。`done` 记录完成过一轮学习，不表示完全掌握。
- 单独修改学习状态保留 `date` 和 `lastmod`；修订笔记或 PDF 不重置已有状态。

2026-09-23 按用户明确确认，将当时全部 76 篇非视频文章（69 篇好文分享、7 篇原创文章）初始化为 `done`，14 篇视频文章保持未标记。此次初始化只记录用户确认的历史状态；后续新增文章仍按上述规则处理。

侧栏的“学习清单”进入 `/learning/`，并显示待学习数量。清单默认展开待学习，已学习和未标记折叠；每个条目集中提供学习状态、普通文章阅读链接和“在 GitHub 编辑”入口。文章使用原有 URL，文章页、首页及普通文章列表保持原有阅读样式，不增加学习记录或学习编辑入口。

可在本地修改 Markdown，也可通过学习清单中对应条目的 GitHub 编辑入口修改同一文件。提交并推送后，等待 GitHub Pages 部署成功，各设备访问网页即可看到更新；浏览器不保存学习状态，站点无需数据库或后端。仓库和学习记录均公开，编辑权限由 GitHub 管理。

采用独立学习清单的 C 方案；三套原型及选择记录保留在 [`codex/learning-list-prototype` 分支](https://github.com/QianKuang8/blog/tree/codex/learning-list-prototype/prototypes/learning-list)，原型代码不进入正式站点。
...
新建视频笔记使用专用模板，自动设置视频栏目和 `learning_status: pending`：

```bash
hugo new content --kind video content/posts/<slug>.md
```

模板生成草稿；发布前仍需补全摘要、正文、来源记录和 PDF 链接，并移除 `draft: true`。
