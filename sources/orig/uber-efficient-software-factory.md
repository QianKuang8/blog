...
domain: "uber.com"
description: "We just published a deep dive on how we keep AI coding costs under control at Uber, even as usage keeps climbing.\n\nSince February, weekly active employees across all functions on our agentic offerings grew 7x, and weekly agent requests grew 9.4x. Over that same stretch, total spend has relatively stabilized.\n\nWe broke total spend into an equation: users, sessions per user, turns per session, requests per turn, tokens per request, price per token, and treated each term as something we could measure and shrink on its own.\n\nA few examples: benchmark-driven model selection instead of a single fixed default, prompt-cache lifetimes matched to how long engineers actually pause mid-session, MCP tools resolved through a CLI instead of preloaded into every session's context, and a session dashboard that automatically flags more than a dozen categories of wasted spend."
author: "Uday Kiran Medisetty"
published_at: "2026-08-27"
retrieved_at: "2026-09-08T15:11:34+08:00"
extractor: "defuddle"
---
...
![Weekly active users and agent requests rose sharply from February to August 2026; costs fluctuated but increased overall.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy9kZGExMDZkMy0wNDM5LTQyN2ItYjc4MC1jMzk5ZGM4NWM3YzgucG5n)

Figure 1: Weekly active users, agent requests, and cost from Feb–Mid Aug 2026, with users deduplicated across tools.

Since adoption, workload mix, and model upgrades are all continuously changing, isolating our own optimization gains means holding one model fixed, since behavior shifts with every upgrade and model family. We did that from February to July: cost per 1,000 model requests is down almost 34% from its peak, and cost per session is down 52% from its June peak.![Unit costs for 2026: cost per 1,000 requests drops 34%, cost per session drops 52% from peak to August.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy9iMjVkMTc1Zi04MjFiLTRiYTYtYTE0OS0xNGU1Yzk1YTI1Y2IucG5n)
...
#### Figure 3: The four layers where agent sessions run. The Cost Equation
...
![Formula for total spend: users × sessions/user × turns/session × requests/turn × tokens/request × price/token.](https://cn-geo1.uber.com/image-proc/crop/smartcrop/udam/format=auto/width=2280/height=654/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy9jYWY5MWU3ZC0wODVkLTQwMTQtOTE4Zi1hZmVmN2Y3YjI5OWIucG5n)
...
| Price / Token | Benchmark-driven, Pareto-optimal model selection  Model defaults |
| --- | --- |
| Tokens / Request | 400K context cap & Medium reasoning effort by default  Prompt Caching  Tool search and CLI-resolved MCP calls  Code-mode batching of tool calls  Gateway-routed SaaS MCPs |
| Requests / Turn | Graph-grounded context  Continuous Skill Optimization |
| Visibility & Education | Live cost counter in the status line  Visibility and spend tiers  Session analysis dashboard |
...
![Scatter plot comparing cost per review and F1 score for 10 model configurations, highlighting Pareto frontier.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy9hM2I2OGQ3OS00ZDFmLTQ3ZDUtOTUyNC01YzBjNDA0YmRiZDIucG5n)
...
![Comparison of 5-minute and 1-hour TTL caching costs for main thread and subagent scenarios, showing cost savings.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy9mNjEzNDE2MC02YmViLTQyNzItOGQ4OC0yYmVlNzkyZDA0MDMucG5n)
...
![Comparison of token usage: installing each server uses ~50-70,000 tokens, tool search plus CLI uses near zero.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy83MWJkMGZiYi0yNDk0LTRmOTItYjE5NC02MTBlM2E5NDA3YmYucG5n)
...
![Comparison of MCP tool use and code-mode workflows, highlighting model turns, token usage, and data flow steps.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy8zMThiN2YyYy0zOGQ2LTQxNWYtYWViMy0xODU5Zjk2NWM1NzMucG5n)
...
### Figure 9: Every SaaS MCP server is exposed behind our MCP gateway to ensure a unified, efficient access pattern. Optimizing Requests / Turn
...
![Query with the graph took 38s and was correct; without the graph took 20m 09s and was wrong.](https://cn-geo1.uber.com/image-proc/resize/udam/format=auto/srcb64=aHR0cHM6Ly90Yi1zdGF0aWMudWJlci5jb20vcHJvZC91ZGFtLWFzc2V0cy8xYzZjNzM3MS1hNGU0LTQ3ZGQtOTQ5Ni0zMTViZWEyYmYxMDIucG5n)
...
### Figure 11: The status line, with the session analyzer and efficiency guide that ship alongside it.Visibility and Spend Tiers
...
### Figure 12: Session-level cost dashboard identifies waste patterns and potential savings.. What’s Next?
