...
侧栏的“学习清单”（`/learning/`）集中展示待学习、已学习和未标记的文章。待学习默认展开，其余分组折叠；首页及普通文章列表保持原样。从清单进入文章后，底部提供状态、GitHub 编辑入口和返回清单链接。

在文章 frontmatter 中填写 `learning_status: pending`（待学习）或 `learning_status: done`（已学习）；省略字段表示未标记。新增视频笔记默认待学习，普通文章只在确认已读后标记，历史文章不自动补标。单独修改状态不更新 `lastmod`，修订文章或 PDF 也不重置状态。

状态随 Markdown 提交、推送，并在 GitHub Pages 部署完成后跨设备同步。可以在本地或 GitHub 编辑文件；学习记录公开，浏览器不保存学习状态。
